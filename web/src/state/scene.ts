...
export type Mode = "p2p" | "bidir" | "race" | "iso" | "multi" | "dispatch";
...
const marker = (g: Graph, node: number, role: MarkerSpec["role"], label?: string): MarkerSpec => ({
  x: g.mx[node],
  y: g.my[node],
  role,
  label,
...
function buildBidir(g: Graph, p: SceneParams): Scene {
  ...
  const thePane: Pane = {
    ...
    routes: route.length ? [route] : [],
  };
  const markers: MarkerSpec[] = [marker(g, p.source, "start"), marker(g, p.target, "end")];
  if (r.meet >= 0) markers.push(marker(g, r.meet, "meet"));
